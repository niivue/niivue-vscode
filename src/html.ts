...
export async function getHtmlForWebview(webview: vscode.Webview, extensionUri: vscode.Uri): Promise<string> {
	...
	return html.replace(noncePlaceholder, nonce).replace(niiVuePlaceholder, niiVue.toString()).replace(scriptUriPlaceholder, scriptUri.toString()).replace(cssUriPlaceholder, cssUri.toString());
}