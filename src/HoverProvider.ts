...
export class LinkHoverProvider implements vscode.HoverProvider {
    ...
    provideHover(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Thenable<vscode.Hover> {
        ...
        return new Promise((resolve, reject) => {
            ...
            const localPattern = new RegExp(`\\b(?:[A-Za-z]:)?\\\\?[\\w\\\\\\s.-]+(${extensions.map(ext => ext.replace('.', '\\.')).join('|')})\\b`, 'i');
            ...
            if (localWordRange) {
                ...
                const commandUri = vscode.Uri.parse(`command:niiVue.openLocal?${encodeURIComponent(JSON.stringify(args))}`);
                const contents = new vscode.MarkdownString(`[Show with NiiVue](${commandUri})`);
                contents.isTrusted = true;
                resolve(new vscode.Hover(contents));
            }
