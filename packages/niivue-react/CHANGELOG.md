...
- 909fd4f: Upgrade the NiiVue core to the v1.0 release candidate (`@niivue/niivue@1.0.0-rc.13`,
...
- 29ef801: Make it easy to find how to cite NiiVue.
...
- cb03cec: Add citation information for the NiiVue wrapper ecosystem paper (Aperture Neuro, 2026, doi:10.52294/001c.167815).
...
- 900a58d: Tighten the spacing between a menu-bar label and its dropdown chevron.
...
- cf47b68: Spell NiiVue with its correct capitalization in the viewer's top bar and About dialog, the JupyterLab "Open With" menu and description, and the web app's bookmarklet label.
...
- 900a58d: Rework the per-pane 4D chrome so the readout, the 4D navigator and the
...
- 978bd39: Show the file name on the tile label when only one image is open. The label shows the part that tells several open images apart, which left a single image with an empty label box; a lone image now shows its file name, and no label box is drawn when there is no name.
- 1acdb31: Make the VS Code viewer's top menu bar 4px thinner (32px).
